...
def deliver_batch(store: MailboxStore, args: argparse.Namespace) -> int:
    ...
    result = store.wait_then_tick(args.instance) if args.wait else store.tick(args.instance)
    ...
    try:
        ...
        output = f"(mailbox agent timeout {args.timeout}s)"
        returncode = 124
        timed_out = True
    atomic_write(batch_path.parent / "agent.log", output.rstrip() + "\n")
    if output:
        print(output.rstrip())
...
def build_parser() -> argparse.ArgumentParser:
    ...
    tick.add_argument("--wait", action="store_true", help="due時刻までmodel外でsleepしてから1回だけ確認")
    ...
    deliver.add_argument("--wait", action="store_true", help="due時刻までmodel外でsleepしてから1回だけ確認")
