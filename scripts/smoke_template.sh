...
for vendor in ("opencode", "kilo"):
    config = json.loads(pathlib.Path(f"{vendor}.jsonc").read_text(encoding="utf-8"))
...
required = {
    "AGENTS.md": ["scripts/agent_mailbox.py", "instance IDとtask ID", "空mailboxではagentを起動しない"],
...
if [[ -f release/template-files.txt ]]; then
  if [[ "${TEMPLATE_SMOKE_RELEASE:-0}" == "1" ]]; then
    ...
    run python3 scripts/build_release_asset.py verify --archive "${release_assets[0]}"
  else
