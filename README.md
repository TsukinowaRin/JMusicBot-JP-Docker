...
単体インストーラーは同じ Release の zip bundle を一時フォルダへダウンロードし、次の安定したアプリデータ領域へ launcher 一式をコピーします。

- Windows: `%LOCALAPPDATA%\JMusicBot-JP-Docker`
- macOS: `~/Library/Application Support/JMusicBot-JP-Docker`
- Linux: `${XDG_DATA_HOME:-~/.local/share}/jmusicbot-jp-docker`

配置先を変えたい場合は、実行前に `JMUSICBOT_INSTALL_DIR` を設定してください。

その後は配置先の `setup.bat` / `setup.command` / `setup.sh` が実行されます。設定ファイルは配置先の `docker-data/config.txt`、Windows では `docker-data\config.txt` です。

Docker 起動後に削除してよいもの / 残すもの:

- 削除してよい: 最初にダウンロードした単体インストーラー、手動でダウンロードした zip、OS の一時フォルダに残った zip。
- 残す: 上記のアプリデータ領域に作られた `JMusicBot-JP-Docker` / `jmusicbot-jp-docker` フォルダ。特に `docker-data/` には `config.txt`、プレイリスト、取得済み jar cache が入ります。
...
単体インストーラは `JMusicBot-JP-Docker-<tag>.zip` を一時フォルダへ取得し、OS 標準のアプリデータ領域へ launcher 一式を配置してから `setup.*` を実行します。zip bundle 内の `Install.*` は互換用の名前として残します。
